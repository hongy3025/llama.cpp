...
TYPES_KV_TURBO = ["GGML_TYPE_Q8_0", "GGML_TYPE_TURBO3_0", "GGML_TYPE_TURBO4_0"]
...
for type_k in TYPES_KV_TURBO:
    for type_v in TYPES_KV_TURBO:
        if type_k == type_v == "GGML_TYPE_Q8_0":
            continue
        with open(f"fattn-vec-instance-{get_short_name(type_k)}-{get_short_name(type_v)}.cu", "w") as f:
            f.write(SOURCE_FATTN_VEC.format(type_k=type_k, type_v=type_v))
